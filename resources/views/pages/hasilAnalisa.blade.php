    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                        ...
                        <tbody>
                            @foreach ($dataSupport as $supp)
                                <tr>
                                    ...
                                    <td>{{ str_pad($supp->kd_produk, 5, '0', STR_PAD_LEFT) }}</td>
                                    <td>{{ $supp->dataProduk($supp->kd_produk)->name }}</td>
                                    <td>{{ $supp->totalTransaksi($supp->kd_produk) }}</td>
                                    <td>
                                        ({{ $supp->totalTransaksi($supp->kd_produk) }} / {{ $totalProduk }})
                                        * 100
                                    </td>
                                    <td>{{ $supp->support }}</td>
                                </tr>
                    ...
                    <table class="table mb-0 table-hover" id="tblDataSupportMin">
                        ...
                        <tbody>
                            @foreach ($dataMinSupport as $minSupp)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ str_pad($minSupp->kd_produk, 5, '0', STR_PAD_LEFT) }}</td>
                    ...
                    <table class="table mb-0 table-hover" id="tblKombinasiItemset">
                        ...
                        <tbody>
                            @foreach ($dataKombinasiItemset as $is)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ str_pad($is->kd_kombinasi, 5, '0', STR_PAD_LEFT) }}</td>
                    ...
                    <table class="table mb-0 table-hover" id="tblMinConfidence">
                        ...
                        <tbody>
                            @foreach ($dataMinConfidence as $is)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ str_pad($is->kd_kombinasi, 0, 5) }}</td>
                                    <td>{{ $is->dataProduk($is->kd_barang_a)->name }}</td>
                                    <td>{{ $is->dataProduk($is->kd_barang_b)->name }}</td>
                                    <td>{{ $is->jumlah_transaksi }}</td>
                                    <td>( {{ $is->jumlah_transaksi }} / {{ $totalProduk }} ) * 100</td>
                            ...
                            @foreach ($dataMinConfidence as $is)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <b>{{ $is->dataProduk($is->kd_barang_a)->name }}</b>,
                                        maka pelanggan juga akan membeli
                                        <b>{{ $is->dataProduk($is->kd_barang_b)->name }}</b>
                                    </td>
                                    <td>{{ $is->support }} %</td>
                                </tr>
                            @endforeach
                ...
                <div>
                    <a href="{{ url('/apriori/analisa/cetak/') }}/{{ $kdPengujian }}" target="new"
                        class="btn btn-primary btn-lg">Cetak Laporan Analisa</a>
                </div>
